...
class TypeScriptConfigCache {
  ...
  getTsConfigForFile(filePath) {
    ...
    // Monorepo: if file is under apps/<name>/ or packages/<name>/, prefer that workspace's tsconfig.
    // This is required so path aliases (e.g. `@/`) defined per-workspace resolve correctly.
    const workspaceMatch = relativePath.match(/^(apps|packages)\/([^/]+)\//);
    if (workspaceMatch) {
      ...
      if (require("fs").existsSync(workspaceTsconfig)) {
...
class QualityChecker {
  ...
  async checkTypeScript() {
    ...
    try {
      ...
      // We invoked createProgram with [this.filePath] only, so cross-module inference is partial.
      // Filter out diagnostics that are unreliable in single-file mode:
      //   7006 — implicit any on parameter (fires when imported generics aren't inferred)
      //   7031 — implicit any on binding element
      //   7034 — implicit any from "no type annotation"
      // The full-project `tsc -p tsconfig.json --noEmit` invocation still catches these correctly.
      const UNRELIABLE_SINGLE_FILE_CODES = new Set([7006, 7031, 7034]);
      const editedFileDiagnostics = (
        diagnosticsByFile.get(this.filePath) || []
      ).filter((d) => !UNRELIABLE_SINGLE_FILE_CODES.has(d.code));
  ...
  async checkESLint() {
    ...
    try {
      ...
      // Filter out "file ignored" meta-warnings — they aren't real issues, they signal
      // the file is outside ESLint's configured paths (common in monorepo workspaces).
      const realMessages = (result.messages || []).filter(
        (m) => !(m.severity === 1 && /File ignored/i.test(m.message || "")),
      );
      const realErrorCount = realMessages.filter(
        (m) => m.severity === 2,
      ).length;
      const realWarningCount = realMessages.filter(
        (m) => m.severity === 1,
      ).length;

      if (realErrorCount === 0 && realWarningCount === 0) {
        log.success("ESLint passed");
        return;
      }

      result.errorCount = realErrorCount;
      result.warningCount = realWarningCount;
      result.messages = realMessages;
    ...
    try {
      ...
      if (/spawn\(|exec\(|execFile\(|fork\(/.test(content)) {
        ...
        if (!/.on\(['"]error['"]/.test(content)) {
          log.warning(
            'Child process spawned without error handling - add .on("error") handler',
          );
          foundIssues = true;
        }
      }
      ...
      if (/\.pipe\(/.test(content)) {
        ...
        const errorHandlerCount = (content.match(/\.on\(['"]error['"]/g) || [])
          .length;

        if (pipeCount > 0 && errorHandlerCount < pipeCount) {
          log.warning(
            "Stream pipe without error handling - each pipe should have error handlers",
          );
          foundIssues = true;
        }
      }
      ...
      if (this.fileType === "cli" || /index\.(ts|js)$/.test(this.filePath)) {
        if (
          ...
          log.warning(
            'Consider adding process.on("unhandledRejection") handler for CLI apps',
          );
        }
      }
      ...
      if (/\/(client|server)\/(stdio|sse|websocket|http)/.test(this.filePath)) {
        if (!content.includes("try") && !content.includes(".catch")) {
          log.warning(
            "Transport implementation should have error handling (try/catch or .catch)",
          );
          foundIssues = true;
        }
      }
